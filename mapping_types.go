// mapping_types.go — Unified controller mapping engine types.
//
// Defines the data model for mapping any input device (MIDI, gamepad, keyboard,
...
func (vt *ValueTransform) applyCurve(normalized float64) float64 {
	param := vt.CurveParam
	if param == 0 {
		param = 2.0 // default curve intensity
	}
	switch vt.Curve {
	...
	case CurveSCurve:
		// Hermite smoothstep for simple S-curve, tanh for parameterized
		if param == 2.0 {
...
type OutputAction struct {
	...
	// Sequence steps (non-recursive: steps use SequenceStep which has no Steps field)
	Steps []SequenceStep `toml:"steps,omitempty" json:"steps,omitempty"`
}

// SequenceStep is one action within a sequence chain.
// It mirrors OutputAction fields but without Steps to avoid recursion.
...
type ProfileMeta struct {
	...
	Type          string   `toml:"type,omitempty" json:"type,omitempty"`     // "mapping", "hardware", "template"
	Device        string   `toml:"device,omitempty" json:"device,omitempty"` // device name
...
// It can be loaded from the unified TOML format or converted from legacy makima format.
