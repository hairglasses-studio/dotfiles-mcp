...
- `dotfiles_oss_score` -- Score a repo's open-source readiness (0-100) across 8 categories
...
- `ops_test_smart` -- Run tests on changed packages only
- `ops_changed_files` -- List changed files with diff stats and Go package mapping
- `ops_analyze_failures` -- Categorize build/test failures with fix suggestions
- `ops_auto_fix` -- Auto-fix mechanical failures: missing deps, imports, unused vars. Dry-run by default
...
- `ops_iterate` -- **Core loop**: build > test > analyze > track iteration
...
- `ops_session_create` -- Create SDLC iteration tracking session
...
- `ops_fleet_diff` -- Fleet-wide changes since a date
